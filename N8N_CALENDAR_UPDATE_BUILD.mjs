import fs from 'node:fs';

const inputPath = process.argv[2] || 'N8N_WORKFLOW_PLINE_V3_CALENDAR_SEARCH_LUNA_IMPORT.json';
const outputPath = process.argv[3] || 'N8N_WORKFLOW_PLINE_V3_CALENDAR_UPDATE_LUNA_IMPORT.json';
const workflow = JSON.parse(fs.readFileSync(inputPath, 'utf8'));
const names = [
  'Calendar Update Route',
  'Calendar Update Read Existing',
  'Calendar Update Inspect Existing',
  'Calendar Update Needed Route',
  'Calendar Update Event',
  'Calendar Update Terminal Readback',
  'Calendar Update Verify Readback',
];

const frozenOpenAi = workflow.nodes.find((node) => node.name === 'OpenAI Chat Model');
if (!frozenOpenAi) throw new Error('missing frozen OpenAI Chat Model');
const frozenOpenAiJson = JSON.stringify(frozenOpenAi);
const frozenModel = frozenOpenAi.parameters?.model?.value || frozenOpenAi.parameters?.model;
if (frozenModel !== 'gpt-5.6-luna' || Number(frozenOpenAi.parameters?.options?.temperature) !== 0.1) {
  throw new Error('refusing to overwrite frozen OpenAI gpt-5.6-luna temperature 0.1 core');
}
if (!frozenOpenAi.credentials || Object.keys(frozenOpenAi.credentials).length === 0) {
  throw new Error('missing frozen OpenAI credential reference');
}

const currentNormalizeTargets = workflow.connections['Normalize Input']?.main?.[0];
const currentUpdateFalseTargets = workflow.connections['Calendar Update Route']?.main?.[1];
const priorNormalizeTargets = currentNormalizeTargets?.[0]?.node === 'Calendar Update Route'
  ? currentUpdateFalseTargets
  : currentNormalizeTargets;
if (!Array.isArray(priorNormalizeTargets) || priorNormalizeTargets.length !== 1 || priorNormalizeTargets[0]?.node !== 'Calendar Search Route') {
  throw new Error('unexpected Luna Search baseline topology');
}

workflow.nodes = workflow.nodes.filter((node) => !names.includes(node.name));
for (const name of names) delete workflow.connections[name];
const normalize = workflow.nodes.find((node) => node.name === 'Normalize Input');
if (!normalize) throw new Error('missing Normalize Input');
const normalizeCode = `if (rawIntent === 'calendar_update') {
  return [{ json: {
    intent: 'calendar_update',
    request_id: String(body.request_id || ''),
    safe_event_hash: String(body.safe_event_hash || '').trim().toLowerCase(),
    calendar_alias: String(body.calendar_alias || ''),
    timezone: String(body.timezone || ''),
    event_reference: String(body.event_reference || '').trim(),
    title: String(body.title || '').trim(),
    location: String(body.location || '').trim(),
    all_day: body.all_day === true,
    date: String(body.date || '').trim(),
    start: String(body.start || '').trim(),
    end: String(body.end || '').trim(),
    readback_only: body.readback_only === true,
    original_title: String(body.original_title || '').trim(),
    original_location: String(body.original_location || '').trim(),
    original_all_day: body.original_all_day === true,
    original_start: String(body.original_start || '').trim(),
    original_end: String(body.original_end || '').trim(),
    allowed_intents: ['calendar_update']
  } }];
}
`;
if (!normalize.parameters.jsCode.includes("if (rawIntent === 'calendar_update')")) {
  normalize.parameters.jsCode = normalize.parameters.jsCode.replace(
    "if (rawIntent === 'calendar_search') {",
    `${normalizeCode}\nif (rawIntent === 'calendar_search') {`,
  );
}

const credentialSource = workflow.nodes.find((node) =>
  ['Calendar Search Events', 'Calendar Read Existing', 'Calendar Terminal Readback'].includes(node.name)
  && node.credentials?.googleCalendarOAuth2Api
);
if (!credentialSource) throw new Error('missing bound Google Calendar credential reference');
const credential = JSON.parse(JSON.stringify(credentialSource.credentials));
const calendar = { __rl: true, value: 'primary', mode: 'list', cachedResultName: 'primary' };
const timezone = { __rl: true, value: 'Asia/Taipei', mode: 'id' };
const routeNode = {
  parameters: {
    conditions: {
      options: { caseSensitive: true, leftValue: '', typeValidation: 'strict', version: 2 },
      conditions: [{
        id: 'pline-v3-calendar-update-route-condition',
        leftValue: '={{ $json.intent }}',
        rightValue: 'calendar_update',
        operator: { type: 'string', operation: 'equals' },
      }],
      combinator: 'and',
    },
    options: {},
  },
  type: 'n8n-nodes-base.if',
  typeVersion: 2.2,
  position: [-16744, 5500],
  id: 'pline-v3-calendar-update-route',
  name: 'Calendar Update Route',
};
const readNode = {
  parameters: {
    operation: 'get',
    calendar,
    eventId: "={{ $('Normalize Input').first().json.event_reference }}",
    options: { timeZone: timezone },
  },
  type: 'n8n-nodes-base.googleCalendar',
  typeVersion: 1.3,
  position: [-16444, 5500],
  id: 'pline-v3-calendar-update-read-existing',
  name: 'Calendar Update Read Existing',
  credentials: credential,
  onError: 'continueRegularOutput',
};
const inspectCode = `const input = $('Normalize Input').first().json;
const event = $json || {};
const failed = Boolean(event.error || event.errorMessage || Number(event.code || 0) >= 400 || !event.id);
const allDay = Boolean(event.start?.date && !event.start?.dateTime);
const current = {
  title: String(event.summary || ''),
  location: String(event.location || ''),
  all_day: allDay,
  start: allDay ? String(event.start?.date || '') : String(event.start?.dateTime || ''),
  end: allDay ? String(event.end?.date || '') : String(event.end?.dateTime || '')
};
const originalMatches = !failed
  && current.title === input.original_title
  && current.location === input.original_location
  && current.all_day === input.original_all_day
  && current.start === input.original_start
  && current.end === input.original_end;
const desiredMatches = !failed
  && current.title === input.title
  && current.location === input.location
  && current.all_day === input.all_day
  && current.start === input.start
  && current.end === input.end;
return [{ json: {
  ...input,
  existing_ok: !failed,
  original_matches: originalMatches,
  desired_matches: desiredMatches,
  safe_to_update: !failed && !input.readback_only && originalMatches && !desiredMatches
} }];`;
const inspectNode = {
  parameters: { jsCode: inspectCode },
  type: 'n8n-nodes-base.code',
  typeVersion: 2,
  position: [-16144, 5500],
  id: 'pline-v3-calendar-update-inspect-existing',
  name: 'Calendar Update Inspect Existing',
};
const neededRoute = {
  parameters: {
    conditions: {
      options: { caseSensitive: true, leftValue: '', typeValidation: 'strict', version: 2 },
      conditions: [{
        id: 'pline-v3-calendar-update-needed-condition',
        leftValue: '={{ $json.safe_to_update }}',
        rightValue: true,
        operator: { type: 'boolean', operation: 'true', singleValue: true },
      }],
      combinator: 'and',
    },
    options: {},
  },
  type: 'n8n-nodes-base.if',
  typeVersion: 2.2,
  position: [-15844, 5500],
  id: 'pline-v3-calendar-update-needed-route',
  name: 'Calendar Update Needed Route',
};
const updateNode = {
  parameters: {
    operation: 'update',
    calendar,
    eventId: '={{ $json.event_reference }}',
    modifyTarget: 'instance',
    useDefaultReminders: true,
    updateFields: {
      start: '={{ $json.start }}',
      end: '={{ $json.end }}',
      allday: "={{ $json.all_day ? 'yes' : 'no' }}",
      location: '={{ $json.location }}',
      sendUpdates: 'none',
      summary: '={{ $json.title }}',
      timezone,
    },
  },
  type: 'n8n-nodes-base.googleCalendar',
  typeVersion: 1.3,
  position: [-15544, 5400],
  id: 'pline-v3-calendar-update-event',
  name: 'Calendar Update Event',
  credentials: credential,
  onError: 'continueRegularOutput',
};
const readbackNode = {
  parameters: {
    operation: 'get',
    calendar,
    eventId: "={{ $('Normalize Input').first().json.event_reference }}",
    options: { timeZone: timezone },
  },
  type: 'n8n-nodes-base.googleCalendar',
  typeVersion: 1.3,
  position: [-15244, 5500],
  id: 'pline-v3-calendar-update-terminal-readback',
  name: 'Calendar Update Terminal Readback',
  credentials: credential,
  onError: 'continueRegularOutput',
};
const verifyCode = `const input = $('Normalize Input').first().json;
const event = $json || {};
const failed = Boolean(event.error || event.errorMessage || Number(event.code || 0) >= 400 || !event.id);
const allDay = Boolean(event.start?.date && !event.start?.dateTime);
const actual = {
  title: String(event.summary || ''),
  location: String(event.location || ''),
  all_day: allDay,
  start: allDay ? String(event.start?.date || '') : String(event.start?.dateTime || ''),
  end: allDay ? String(event.end?.date || '') : String(event.end?.dateTime || '')
};
const verified = !failed
  && actual.title === input.title
  && actual.location === input.location
  && actual.all_day === input.all_day
  && actual.start === input.start
  && actual.end === input.end;
return [{ json: {
  request_id: input.request_id,
  intent: 'calendar_update',
  status: verified ? 'completed' : 'failed',
  reason: verified ? '' : (failed ? 'calendar_update_readback_failed' : 'calendar_update_readback_mismatch'),
  readback_verified: verified,
  title: verified ? actual.title : '',
  location: verified ? actual.location : '',
  all_day: verified ? actual.all_day : false,
  start: verified ? actual.start : '',
  end: verified ? actual.end : ''
} }];`;
const verifyNode = {
  parameters: { jsCode: verifyCode },
  type: 'n8n-nodes-base.code',
  typeVersion: 2,
  position: [-14944, 5500],
  id: 'pline-v3-calendar-update-verify-readback',
  name: 'Calendar Update Verify Readback',
};

workflow.nodes.push(routeNode, readNode, inspectNode, neededRoute, updateNode, readbackNode, verifyNode);
workflow.connections['Normalize Input'] = { main: [[{ node: 'Calendar Update Route', type: 'main', index: 0 }]] };
workflow.connections['Calendar Update Route'] = {
  main: [
    [{ node: 'Calendar Update Read Existing', type: 'main', index: 0 }],
    priorNormalizeTargets,
  ],
};
workflow.connections['Calendar Update Read Existing'] = { main: [[{ node: 'Calendar Update Inspect Existing', type: 'main', index: 0 }]] };
workflow.connections['Calendar Update Inspect Existing'] = { main: [[{ node: 'Calendar Update Needed Route', type: 'main', index: 0 }]] };
workflow.connections['Calendar Update Needed Route'] = {
  main: [
    [{ node: 'Calendar Update Event', type: 'main', index: 0 }],
    [{ node: 'Calendar Update Terminal Readback', type: 'main', index: 0 }],
  ],
};
workflow.connections['Calendar Update Event'] = { main: [[{ node: 'Calendar Update Terminal Readback', type: 'main', index: 0 }]] };
workflow.connections['Calendar Update Terminal Readback'] = { main: [[{ node: 'Calendar Update Verify Readback', type: 'main', index: 0 }]] };
workflow.connections['Calendar Update Verify Readback'] = { main: [[{ node: 'Respond to Webhook', type: 'main', index: 0 }]] };

workflow.calendar_update_gate_contract = {
  gate: 'CALENDAR_UPDATE_GATE',
  calendar_alias: 'authorized_test',
  calendar_backend: 'primary',
  timezone: 'Asia/Taipei',
  actor_pending_ttl_seconds: 600,
  confirmation_required: true,
  original_candidate_match_required: true,
  terminal_readback_required: true,
  calendar_delete_effect: 0,
  create_nodes_changed: false,
  search_nodes_changed: false,
  memo_nodes_changed: false,
  openai_core_model_status: 'preserved_gpt-5.6-luna',
  retry_evidence_limitation: 'formal_same_webhook_live_replay_not_executed',
};
workflow.workflow_version_name = 'Calendar Update Actor Snapshot Confirmation Gate';
if (JSON.stringify(workflow.nodes.find((node) => node.name === 'OpenAI Chat Model')) !== frozenOpenAiJson) {
  throw new Error('frozen OpenAI Chat Model changed during Update merge');
}
fs.writeFileSync(outputPath, `${JSON.stringify(workflow, null, 2)}\n`);
console.log(JSON.stringify({
  input: inputPath,
  output: outputPath,
  nodes: workflow.nodes.length,
  connection_sources: Object.keys(workflow.connections).length,
  calendar_update_nodes: names.length,
  openai_core_model_status: 'preserved_gpt-5.6-luna',
}));
